...
class DrupalEditArticleBasicAuth extends Component {
  ...
  getArticle(nid) {
    var url = this.baceUrl + 'node/' + nid + '?_format=json';
    fetch(url)
      .then(response => response.json())
      .then(data => {
        console.log(data);
        if (data.body[0].value) {
          this.setState({body: data.body[0].value});
        }
        if (data.title[0].value) {
          this.setState({title: data.title[0].value});
        }
        if (data.field_image.length !== 0) {
          if (data.field_image[0].target_id) {
            this.setState({'fid': data.field_image[0].target_id});
            // Get image.
            this.getImage(data.field_image[0].target_id);
          }
        }
  ...
  getImage(fid) {
    var url = this.baceUrl + 'file/' + fid + '?_format=hal_json';
    fetch(url, {
      method: 'GET',
        headers: {
      ...
      .then(data => {
        console.log(data);
        // @TODO need to find out how to set this better.
        var data = 'data:image/png;base64,' + data.data[0].value;
        this.setState({file_string: data});
      }).catch(err => console.error(url, err.toString()))
  ...
  postNewArticle() {
    ...
    var url = this.baceUrl + 'node/'+ this.state.nid +'?_format=json';
    fetch(url, {
      method: 'PATCH',
      headers: {
...
export default DrupalEditArticleBasicAuth;